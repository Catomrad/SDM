...
class XMLRepository(ABCRepository):
    ...
    def find_client(self, querry):
        result = []
        root, _ = self.create_or_open_xml()
        with open(self.file_path, "r") as file:
            clients = root.find('Client')
            for child in clients:
                ...
                if name == querry:
                    result.append([id, name, age, email, addr])
        return result
    def find_product(self, querry):
        result = []
        root, _ = self.create_or_open_xml()
        with open(self.file_path, "r") as file:
            clients = root.find('Product')
            for child in clients:
                id = child.find('id').text
                name = child.find('name').text
                category = child.find('category').text
                material = child.find('material').text
                price = child.find('price').text
                if name == querry:
                    result.append([id, name, category, material, price])
        return result
    def find():
        pass
